function AgentAudioVisualizerAura({
  ...
  const { speed, scale: scale2, amplitude, frequency, brightness } = useAgentAudioVisualizerAura(
    state,
    audioTrack
  );
...
function FridayOrb({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "pointer-events-none absolute inset-x-0 top-7 z-30 flex justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rounded-[24px] border border-border bg-black p-1.5 shadow-2xl backdrop-blur-xl", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
    AgentAudioVisualizerAura,
    {
      ...
      state,
      audioTrack,
      className: "m-0 max-h-15 max-w-15 p-0"
    }
  ) }) });
}
const welcomeVoice = "" + new URL("welcome-BHNW8UGm.mp3", import.meta.url).href;
const permissionsVoice = "" + new URL("permissions-BWEuifUl.mp3", import.meta.url).href;
const byokVoice = "" + new URL("byok-CCRzyynN.mp3", import.meta.url).href;
const onboardingScript = {
  welcome: {
    voice: welcomeVoice,
    subtitle: "Welcome. I'm Friday. Let's get you set up — it'll only take a minute."
  },
  permissions: {
    voice: permissionsVoice,
    subtitle: "First, I'll need a little access — so I can see your screen, and hear your voice."
  },
  byok: {
    voice: byokVoice,
    subtitle: "One last thing: your key. It stays encrypted on this machine, and never leaves it. Then we begin."
  }
};
const MUSIC_VOLUME = 0.8;
const DUCK_VOLUME = 0.3;
const DUCK_RAMP = 0.35;
const VOICE_DELAY = 2e3;
const CROSSFADE = 0.5;
const OUTRO_TAIL = 8e3;
const OUTRO_DIM = 3e3;
